  ...
  reducers: {
    getProject: (state, action) => {
      if (action.payload === "Tasks") {
        state.currentProject = {} as TProject;
      } else {
...
export const addTodoIntoProject = createAsyncThunk(
  "projects/addTodoIntoProject",
...
export const getCurrentProject = createAsyncThunk(
  ...
  async (projectId: string) => {
    try {
      const data = (await firebaseProvider.getDocById(
        "projects",
        projectId
      )) as TProject;
      return data;
